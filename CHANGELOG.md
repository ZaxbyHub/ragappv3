...
- **Settings / logs / auth hardening (issue #387, #403)**: three backend security fixes from the #118 deep-dive residuals:
  ...
  - **Known residuals (out of scope, tracked for follow-up)**: `GET /settings/connection` (`settings.py:935`) also returns raw infra URLs to any authenticated user (pre-existing, same class as MED-4, AC1 literally pins only `GET /settings`); `uid` is still logged raw at several `email_service.py` sites (issue pins only `vault_name`); `_sanitize_log_value` does not strip Unicode bidi marks; **the pre-existing register/login/refresh routes have the broken decorator order (`@limiter.limit` above `@router.post`) which silently disables their rate limits** — change-password is fixed here with the correct order, but a follow-up should correct the other three.
