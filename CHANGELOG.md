...
- **Settings / logs / auth hardening (issue #387, #403)**: three backend security fixes from the #118 deep-dive residuals:
  - **MED-4 — `GET /settings` infra-URL redaction**: `get_settings` now calls `_redact_infra_for_non_admin(settings_dict, role)` which blanks the ten infra-URL and model-name fields (`ollama_embedding_url`, `ollama_chat_url`, `instant_chat_url`, `reranker_url`, `wiki_llm_curator_url`, `embedding_model`, `chat_model`, `instant_chat_model`, `reranker_model`, `wiki_llm_curator_model`) for callers below the admin role (`UserRole.level < admin`), so a viewer/member cannot map the deployment's internal hosts/ports or model inventory. Admin/superadmin callers (and admin-secret / service-account callers, hardcoded superadmin) still see full values to populate the Settings form. The helper also coerces `wiki_llm_curator_enabled` to `False` for non-admins so the frontend's required-when-enabled validator does not fire on a redacted form, and strips the redacted fields' entries from `effective_sources` so the kv/env/default provenance map does not leak a one-bit "is this infra URL configured" hint per field. POST/PUT `/settings` are unchanged (already admin-gated via `require_role("admin")`).
  - **MED-8 — `POST /auth/change-password` rate limit**: added `@limiter.limit("10/minute")` to the route (matching the login-tier policy and the issue's stated `"10/minute"`), closing an authenticated brute-force vector against `current_password`. The route already takes `request: Request` so no signature change was needed. **Decorator order is `@router.post` outermost, `@limiter.limit` directly below** — this is load-bearing: the reverse order (limiter above router) silently breaks enforcement because the router stores the unwrapped function and the limit check never fires. (The pre-existing register/login/refresh routes had the broken order at the time of #387/#403; **issue #659 corrected them in 1d142b43**.) When the limit is exceeded, the decorator wrapper raises `RateLimitExceeded` (a Starlette `HTTPException` with `status_code=429`), which the framework's default exception handler converts to a 429 response.
  - **LOW-2 — `vault_name` log sanitization**: `email_service.py` now routes `vault_name` through `self._sanitize_log_value(...)` at the three interpolation sites (`:436` debug log in `_process_email`, `:684` success-branch debug log in `_resolve_vault_id`, `:688` the `ValueError` raised for a missing vault). The `:688` sanitization also covers the downstream `logger.error(f"... {e}")` path at `:259` (the exception message is sanitized at the source). Prevents log-line injection via a forged email subject tag.
  - **Tests**: 10 new tests across `test_settings.py` (`TestSettingsInfraRedaction` — viewer/member/admin/superadmin with real TestClient GETs), `test_auth_rate_limiting.py` (source-scan decorator presence + in-process runtime `RateLimitExceeded` enforcement + real-route wiring check), `test_email_service.py` (`TestVaultNameLogSanitization` — success-log, not-found-error, and structural source-grep guard). The source-grep regex was tightened to catch `{vault_name!r}` / `{vault_name:10}` / `{ vault_name }` forms. `TestSettingsInfraRedaction.setUp` uses `addCleanup` for all singleton mutations so a setUp abort cannot poison the module. Bandit baseline regenerated for the one-line auth.py decorator insertion (140 → 140 findings, pure mechanical line shift).
  - **Known residuals (out of scope, tracked for follow-up)**: ~~`GET /settings/connection` (`settings.py:935`) also returns raw infra URLs to any authenticated user (pre-existing, same class as MED-4, AC1 literally pins only `GET /settings`)~~ **resolved in #660**; `uid` is still logged raw at several `email_service.py` sites (issue pins only `vault_name`); `_sanitize_log_value` does not strip Unicode bidi marks; ~~the pre-existing register/login/refresh routes have the broken decorator order (`@limiter.limit` above `@router.post`) which silently disables their rate limits — change-password is fixed here with the correct order, but a follow-up should correct the other three~~ **resolved in #659** (commit `1d142b43`).

- **`GET /settings/connection` infra-URL redaction (issue #660)**: the connection-test endpoint now applies role-aware redaction consistent with MED-4's `GET /settings` contract. Callers below the admin role receive the target name (`embeddings`/`chat`/`reranker`) instead of the configured endpoint URL, get no `model` entry in the local-reranker fallback, and have error details reduced to the exception type name after the classification prefix (`SSRF blocked: URLBlocked`, `transport failure: ConnectError`) so no configured host, resolved address, or URL fragment is disclosed; `ok`/`status` remain for member diagnostics and admin/superadmin responses are unchanged. The endpoint previously echoed raw infra URLs (embeddings, chat, reranker) to any authenticated user — a known residual of MED-4 named in the list above since PR #418.

- **Prompt-injection hardening extension (issue #402)**: Extended the ENH-013 XML-escape + SECURITY BOUNDARY pattern (originally landed in `prompt_builder.py` for #209) to the two LLM call sites that were missed. `contextual_chunking.py` `_build_prompt` now `_header_escape`s the filename and wraps both `document_text` and `chunk_text` in `<document>` tags with `_xml_escape`, plus a SECURITY BOUNDARY directive in the system message. `wiki_curator.py` `build_user_prompt` now `_header_escape`s `chunk_id` and the `existing_entities_brief` entity names, wraps the source snippet AND the `deterministic_summary` JSON in `<source_passages>` tags with `_xml_escape` (using `quote=False` on the JSON blob so the JSON `"` delimiters are preserved), and `_CURATOR_SYSTEM_PROMPT` carries a SECURITY BOUNDARY directive. The curator's `_quote_matches` provenance check now also accepts the HTML-unescaped form of an LLM-returned quote, so a model that echoes the rendered tokens (e.g. `&amp;` for `&`) does not cause a legitimate special-character claim to be falsely rejected — provenance is preserved (the unescaped quote must still be a real substring of raw `source_text`). Closes #386 (retrieval_evaluator was already fixed) and #392 MEDIUM-3. Regression tests (`TestBuildPromptInjectionDefense`, `TestBuildUserPromptInjectionDefense`, plus two `TestQuoteMatches` cases) verify that payloads containing `</document>`/`</source_passages>`, injected `<instruction>`/`<user_query>` tags, and newline-forged `[SYSTEM]` directive lines cannot break the prompt boundary; each positive test fails on pre-fix code. A residual same-class site in `chunk_enrichment.py` (default-off) is tracked separately in #416.
...
- **Phase 1 — password & session security hardening (issue #271)**: eleven security fixes and test-quality improvements:
  ...
  - **FR-003 (MEDIUM B1-3)**: Refresh-token reuse detection in `_rotate_refresh_token_block` now detects when a previously-rotated refresh token is presented again. On detection: (a) all `user_sessions` for that user are deleted (family revocation), (b) an `auth.refresh_reuse_detected` security audit event is emitted, (c) the active-user cache is invalidated, (d) HTTP 401 is returned to the client. The IntegrityError branch now also rolls back its EXCLUSIVE transaction before returning the connection to the pool (pool-poisoning fix), and reuse-detection audit events capture the proxy-aware client IP/UA.
  - **FR-004 (MEDIUM B5-001)**: `sak_`-prefixed Bearer service-account keys now authenticate to the nine read-only document/search/tags routes via the combined `get_current_user_or_service_account` dependency. SA principals map to a superadmin-equivalent role for read operations, and their DB lookup runs off the event loop (`asyncio.to_thread`).
  - **FR-005 (LOW C1-3)**: `test_deps_auth_adversarial.py` assertions narrowed from `in [401, 403]` to exactly `== 403` for the `users_enabled=False` admin-token path; new `users_enabled=True` cases assert `== 401`.
  - **FR-006 (LOW A1-1)**: `get_user_organizations` enforces a shared-organization intersection guard for non-superadmin admins (403 when the target user shares no org with the caller).
  - **FR-007 (LOW B1-2)**: A `password_changed_at` epoch column is bumped by `change_password` and `admin_reset_password`; `get_current_active_user` rejects access tokens whose `iat` is strictly less than that epoch, forcing re-authentication after a password change. (Same-second `iat == epoch` tokens are accepted per the RFC 7519 NumericDate 1-second floor; refresh sessions are deleted so only short-lived access tokens can survive.)
  ...
  - **FR-010 (LOW C4-3)**: Magic `==6` count replaced with a per-call-site regex assertion in `test_path_prefix.py`.
  ...
  - **Test coverage (issue #398)**: The refresh-reuse audit `reason` metadata field is now asserted; a same-second `iat == password_changed_at` boundary test and negative tests for non-`sak_` Bearer keys and revoked service accounts were added.
...
- **LanceDB Expr API migration for FTS vault_id filters (Phase 1, issue #204 follow-up)**: FTS search paths in `vector_store.py` now use the LanceDB Expr API (`col("vault_id").eq(lit(vault_id))`) for type-safe vault_id filtering instead of string interpolation. The dense vector arm retains string interpolation via `_lance_escape()` (SQL-standard doubled single-quote escaping) as a documented fallback. Regression tests in `test_vector_store_search_guard.py` cover Expr API usage, SQL injection prevention via `_lance_escape()`, and cross-vault leakage prevention. Test stubs for `lancedb.expr.col`/`lit` added to `conftest.py`.

- **Phase 3 — BackgroundProcessor enrichment resilience (issue #204 follow-up)**:
  - **Enrichment retry with exponential backoff**: `EnrichmentTaskItem` gained an `attempt` field, and `_enrichment_worker_loop` now retries failed enrichment jobs up to `max_retries` times with exponential backoff before marking the task permanently failed, matching the existing `TaskItem` retry pattern. A shutdown guard prevents requeueing during processor shutdown.
  - **Stranded-row recovery tests**: Added `TestStrandedRowRecovery` covering startup recovery of stranded `status='processing'` rows, verifying reset to `pending/queued` or `error` when the file is missing.
  - **Graceful-shutdown-under-load test**: Added `TestGracefulShutdownUnderLoad` verifying the ingestion queue drains and workers cancel cleanly under load.
...
- **SQLite pool release-time rollback (issue #548, audit finding C03)**: `SQLiteConnectionPool.release_connection` now rolls back any open transaction before a connection returns to the pool (covering both the queue path and the pool-full close path), so a handler that returns between a DML statement and its `commit()` can no longer poison the pool: the next borrower's `commit()` no longer durably persists the first handler's abandoned rows (phantom commit), and `PRAGMA foreign_keys` enforcement is restored on re-borrow (the PRAGMA silently no-ops inside a stale transaction). The rollback is a no-op for clean connections and can never raise (the `in_transaction` access and `rollback()` are wrapped separately, so an already-closed connection degrades to a no-op instead of masking a handler's exception in `get_db`'s bare `finally`). **New operator-visible log event**: `WARNING pool_release_rollback sqlite_path=<path> dirty_release=1` (plus `rollback_failed=1` if the rollback itself errors) — each occurrence names a call site that still returns a connection mid-transaction. `MaintenanceService.set_flag`'s optimistic-lock version-miss branch (the one live trigger: a concurrent maintenance-flag toggle race) now explicitly rolls back its no-op UPDATE instead of relying on the pool backstop. No settings, schema, or public-API changes. Regression suite: `backend/tests/test_pool_release_rollback.py` (11 tests).
