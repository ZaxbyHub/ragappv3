...
- **Phase 1 — password & session security hardening (issue #271)**: eleven security fixes and test-quality improvements:
  ...
  - **FR-004 (MEDIUM B5-001)**: `sak_`-prefixed Bearer service-account keys now authenticate to the eight read-only document/search/tags routes via the combined `get_current_user_or_service_account` dependency. SA principals map to a superadmin-equivalent role for read operations, and their DB lookup runs off the event loop (`asyncio.to_thread`).
