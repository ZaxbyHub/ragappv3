...
- **SQLite connection lifetime fix (issues #301, #205)**: Auth/policy SQLite connection is now released before SSE body generation begins on `POST /api/chat/stream` and `GET /api/wiki/events`. The new `get_chat_stream_auth_context` and `get_wiki_events_auth_context` dependencies acquire the connection inside a `try/finally` so the pool slot is freed for the duration of LLM streaming. All six route modules (chat, folders, kms, memories, tags, wiki) migrated from standalone `evaluate_policy(...)` to the DI `Depends(get_evaluate_policy)` so policy checks reuse the request-scoped connection. Closes #301 and #205. Regression coverage: `test_chat_route_policy_di.py` patches BOTH the chat-module and deps-module pool getters so pre-fix code (which routed through `Depends(get_evaluate_policy)`) and post-fix code (which routes through `app.api.routes.chat.get_pool`) are both exercised; AST-based structural test (`test_route_policy_di_no_standalone.py`) prevents re-introduction of standalone `evaluate_policy` calls.
