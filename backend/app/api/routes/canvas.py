...
from app.services.canvas_store import (
    CANVAS_VERSION_CONFLICT,
    CANVAS_VERSION_LIMIT,
    CanvasStore,
)
...
class CreateCanvasArtifactRequest(BaseModel):
    ...
    source_refs: Optional[List[Dict[str, Any]]] = Field(default=None, max_length=50)
...
def _check_content(content: str) -> None:
    ...
    The cap is BYTE-based (utf-8), matching documents.py upload limits and the
    `canvas_max_artifact_kb` name — a code-point count would let multibyte
    content exceed the cap by up to 4x.
    """
    if not content or not content.strip():
        raise HTTPException(status_code=422, detail="canvas_content_required")
    if len(content.encode("utf-8")) > settings.canvas_max_artifact_kb * 1024:
...
async def create_session_artifact(
    ...
    source_refs_json = json.dumps(body.source_refs or [])
    if len(source_refs_json) > 8192:
        raise HTTPException(status_code=422, detail="canvas_source_refs_too_large")
    ...
    if not artifact:
        # Cascade-deleted between insert commit and re-read (session DELETE
        # race) — a clean 404 instead of an unhandled KeyError.
        raise HTTPException(status_code=404, detail="canvas_artifact_not_found")
...
async def save_canvas_version(
    ...
    if version is CANVAS_VERSION_LIMIT:
        raise HTTPException(status_code=413, detail="canvas_version_limit_reached")
    return _serialize_version(version)
...
async def download_canvas_version(
    ...
    if artifact["turn_id"] is not None:
        # Client-controlled string: URL-quote so non-ASCII cannot break the
        # latin-1 header encode inside Starlette (permanent 500 — no DELETE).
        headers["X-Canvas-Turn-Id"] = _url_quote(str(artifact["turn_id"]))
