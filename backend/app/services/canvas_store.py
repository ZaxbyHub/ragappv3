...
# Sentinel returned by append_version when the artifact already has
# settings.canvas_max_versions versions (bounded history growth).
CANVAS_VERSION_LIMIT: Any = object()
...
class CanvasStore:
    ...
    def list_versions(self, artifact_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = conn.execute(
                "SELECT id, artifact_id, version_no, name, content_sha256, "
            ...
            for row in cursor.fetchall():
                # Metadata only — content bodies never leave the DB on list
                # (they are fetched per-version via get_version).
                summaries.append(
                    {
                        "id": row["id"],
                        "artifact_id": row["artifact_id"],
        ...
        try:
            ...
            try:
                version_count = conn.execute(
                    "SELECT COUNT(*) FROM canvas_versions WHERE artifact_id = ?",
                    (artifact_id,),
                ).fetchone()[0]
                if version_count >= settings.canvas_max_versions:
                    ...
                    limit_reached = True
                    conn.rollback()
                if not limit_reached and base_version_no is not None:
                    cursor = conn.execute(
