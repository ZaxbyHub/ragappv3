...
class CanvasTestBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            # 4: admin role — baseline write without vault membership
            # (distinct from superadmin's blanket admin per authz_policy)
            conn.execute(
                "INSERT INTO users (id, username, hashed_password, full_name, role, is_active) "
                "VALUES (?, ?, ?, ?, ?, 1)",
                (4, "admin1", PW, "Admin One", "admin"),
            )
            conn.commit()
        finally:
            self._connection_pool.release_connection(conn)
    ...
    def _admin_headers(self):
        return {"Authorization": f"Bearer {self._token(4, 'admin1', 'admin')}"}
...
class TestCanvasEditRange(CanvasTestBase):
    ...
    def test_oversize_multibyte_content_rejected_by_bytes(self):
        """F-02: the cap is BYTE-based — multibyte content under the char cap
        but over the byte cap must 413."""
        session_id = self._make_session()
        # 300k CJK chars = ~900KB utf-8 bytes > 512KB byte cap, while the
        # code-point count (300k) stays under the char-cap threshold.
        content = "語" * 300_000
    ...
    def test_source_refs_bounds(self):
        """F-04: >50 entries rejected (Pydantic); oversized serialized payload rejected (422)."""
        session_id = self._make_session()
        too_many = self._create_artifact(
            session_id,
            source_refs=[{"source_id": f"s{i}", "title": "t"} for i in range(51)],
        )
        self.assertEqual(too_many.status_code, 422)

        huge = self._create_artifact(
            session_id,
            source_refs=[{"source_id": "x" * 100, "title": "y" * 100} for _ in range(40)],
        )
        self.assertEqual(huge.status_code, 422)
        self.assertEqual(huge.json()["detail"], "canvas_source_refs_too_large")

    def test_version_limit_reached_413(self):
        """F-08: appending beyond canvas_max_versions returns 413 (history preserved)."""
        session_id = self._make_session()
        ...
        original = app_settings.canvas_max_versions
        app_settings.canvas_max_versions = 3
        try:
            # v1 exists; limit 3 allows exactly 2 more appends.
            for i in range(2):
                ok = self.client.post(
                    f"/api/canvas/artifacts/{uid}/versions",
                    json={"content": f"v{i+2}" + chr(92) + "n", "base_version_no": i + 1},
                    headers=self._super_headers(),
                )
                self.assertEqual(ok.status_code, 200)
            capped = self.client.post(
                f"/api/canvas/artifacts/{uid}/versions",
                json={"content": "over" + chr(92) + "n", "base_version_no": 3},
                headers=self._super_headers(),
            )
            self.assertEqual(capped.status_code, 413)
            self.assertEqual(capped.json()["detail"], "canvas_version_limit_reached")
        finally:
            app_settings.canvas_max_versions = original
        # History preserved: v1 + 2 appends, nothing destroyed.
        versions = self.client.get(
            f"/api/canvas/artifacts/{uid}/versions", headers=self._super_headers()
        ).json()["versions"]
        self.assertEqual(len(versions), 3)

    def test_turn_id_non_ascii_download_ok(self):
        """F-09: non-ASCII turn_id is URL-quoted in X-Canvas-Turn-Id — no 500."""
        session_id = self._make_session()
        message_id = self._make_message(session_id, turn_id="turn-語-🎉")
        uid = self._create_artifact(
        ...
        self.assertEqual(response.status_code, 200)
        from urllib.parse import unquote
        self.assertEqual(unquote(response.headers["X-Canvas-Turn-Id"]), "turn-語-🎉")

    def test_edit_range_fence_only_replacement_422(self):
        """N-23: a fence-only model replacement splices to empty content → 422."""
        session_id = self._make_session()
        uid = self._create_artifact(session_id, content="line1\nline2\n").json()["artifact_uid"]
        self._install_llm("```\n```")
        response = self.client.post(
            f"/api/canvas/artifacts/{uid}/edit-range",
            json={"start_line": 1, "end_line": 2, "instruction": "clear it", "base_version_no": 1},
            headers=self._super_headers(),
        )
        self.assertEqual(response.status_code, 422)
...
class TestCanvasAuthorization(CanvasTestBase):
    def test_canvas_authz_matrix(self):
        ...
        # Admin role (distinct from superadmin): baseline write without
        # membership (authz_policy admin baseline_level = write).
        admin_ok = self.client.get(
            f"/api/canvas/artifacts/{uid}", headers=self._admin_headers()
        )
        ...
        self.assertEqual(admin_write.status_code, 200)
