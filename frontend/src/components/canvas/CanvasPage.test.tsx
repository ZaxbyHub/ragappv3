...
describe("CanvasPage restore", () => {
  it("restoring an old version appends a restored version via the confirm dialog", async () => {
    ...
    getCanvasVersionMock.mockResolvedValue(makeVersion(1, V1_CONTENT, "created"));
    listCanvasVersionsMock
...
describe("CanvasPage compare tab", () => {
  it("renders real diffLines rows with expected removed and added lines", async () => {
    ...
    getCanvasVersionMock.mockImplementation(async (_uid: string, versionNo: number) =>
      makeVersion(versionNo, versionNo === 1 ? V1_CONTENT : V2_CONTENT, "user_edit")
    );
...
describe("CanvasPage downloads", () => {
  it("downloads the selected version and compares blob BYTES to the version content", async () => {
    mockHappyPath();
    downloadCanvasVersionMock.mockResolvedValue({
      blob: new Blob([V2_CONTENT], { type: "text/plain" }),
      filename: "demo.py",
    });
    ...
    // N-25: byte-level compare via arrayBuffer, not text() decoding. Plain
    // arrays because Uint8Array toEqual can false-fail across realms in jsdom.
    const expected = new TextEncoder().encode(V2_CONTENT);
    const actual = new Uint8Array(await capturedBlobs[0].arrayBuffer());
  ...
  // N-24: exporting while viewing a historical version must target THAT
  // version, not the artifact's current one.
  it("exports the selected historical version's manifest", async () => {
    mockHappyPath();
    exportCanvasManifestMock.mockResolvedValue({
      artifact_uid: "cav_test1",
      kind: "code",
    ...
    await waitFor(() => expect(exportCanvasManifestMock).toHaveBeenCalledWith("cav_test1", 1));
    await waitFor(() => expect(capturedBlobs).toHaveLength(1));
    const manifest = JSON.parse(await capturedBlobs[0].text());
    expect(manifest.version_no).toBe(1);
    expect(manifest.content).toBe(V1_CONTENT);
    const anchor = anchorClick.mock.instances[0] as HTMLAnchorElement;
    expect(anchor.download).toBe("demo-py-v1-manifest.json");
    anchorClick.mockRestore();
  });
});
...
// ===========================================================================
// F-07: navigate-with-dirty guard (beforeunload + confirmLeave + anchor capture)
// ===========================================================================

const DIRTY_SUFFIX = " // unsaved change";

describe("CanvasPage navigate-with-dirty guard", () => {
  function makeDirty(): void {
    const editor = screen.getByLabelText("Canvas content editor") as HTMLTextAreaElement;
    fireEvent.change(editor, { target: { value: editor.value + DIRTY_SUFFIX } });
  }
  ...
  it("registers confirmLeave and blocks beforeunload while dirty", async () => {
    mockHappyPath();
    renderCanvasPage();
    await screen.findByLabelText("Canvas content editor");
    expect(useNavigationGuardStore.getState().confirmLeave).toBeNull();
    expect(dispatchBeforeUnload()).toBe(false);

    makeDirty();
    const confirmLeave = useNavigationGuardStore.getState().confirmLeave;
    expect(confirmLeave).not.toBeNull();
    expect(confirmLeave?.()).toBe(true);
    expect(dispatchBeforeUnload()).toBe(true);
  });

  it("clears confirmLeave after a successful save", async () => {
    mockHappyPath();
    saveCanvasVersionMock.mockResolvedValue(makeVersion(3, V2_CONTENT, "user_edit"));
    renderCanvasPage();
    await screen.findByLabelText("Canvas content editor");
    makeDirty();
    expect(useNavigationGuardStore.getState().confirmLeave).not.toBeNull();

    fireEvent.click(screen.getByTestId("canvas-save-button"));
    await waitFor(() => expect(saveCanvasVersionMock).toHaveBeenCalledTimes(1));
    await waitFor(() => expect(useNavigationGuardStore.getState().confirmLeave).toBeNull());
  });

  it("blocks in-app anchor navigation while dirty via the capture handler", async () => {
    mockHappyPath();
    renderCanvasPage();
    await screen.findByLabelText("Canvas content editor");
    makeDirty();
    ...
    anchor.setAttribute("href", "/chat/7");
    document.body.appendChild(anchor);
    // The capture handler only blocks when the user declines to leave;
    // setup.ts's global window.confirm mock returns true (leave allowed).
    const confirmSpy = vi.spyOn(window, "confirm").mockReturnValue(false);
    const click = new MouseEvent("click", { bubbles: true, cancelable: true });
    anchor.dispatchEvent(click);
    expect(click.defaultPrevented).toBe(true);

    // Accepting the confirm leaves navigation unprevented (guard stays wired).
