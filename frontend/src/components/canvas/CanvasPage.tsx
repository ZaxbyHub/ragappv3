...
export default function CanvasPage() {
  ...
  // Guards async mutations against artifact switches: a save/restore/edit
  // captured under an old artifactUid must not touch the new artifact's state.
  const activeUidRef = useRef(artifactUid);
  useEffect(() => {
    activeUidRef.current = artifactUid;
  }, [artifactUid]);
  ...
  const activeContent = viewingOldVersion ? oldVersionQuery.data?.content ?? null : editorText;
  ...
  const handleSave = async (force: boolean) => {
    ...
    try {
      ...
      if (activeUidRef.current !== artifactUid) return; // artifact switched mid-save
  ...
  const handleRestore = async (versionNo: number) => {
    ...
    try {
      ...
      if (activeUidRef.current !== artifactUid) return; // artifact switched mid-restore
      applyNewVersion(version);
      await refreshArtifact();
  ...
  const handleEditRange = async () => {
    ...
    try {
      ...
      if (activeUidRef.current !== artifactUid) return; // artifact switched mid-edit
      applyNewVersion(version);
      setEditRangeOpen(false);
      await refreshArtifact();
  ...
  return (
    ...
    >
      ...
      <Dialog
        open={editRangeOpen}
        onOpenChange={(open) => {
          setEditRangeOpen(open);
          if (!open) editRangeAbortRef.current?.abort(); // Stop closes == abort
        }}
      >
