...
describe("canvas detail payload normalization", () => {
  ...
  it("getCanvasVersion passes the bare version object through (contract delta #5)", async () => {
    mockGet.mockResolvedValueOnce({ data: RAW_VERSION });
    const result = await getCanvasVersion("cav_test1", 2);
    expect(result).toEqual(RAW_VERSION);
  });
