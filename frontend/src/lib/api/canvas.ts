...
export async function getCanvasVersion(
  ...
  // The version endpoint returns a BARE version object, not the
  // {artifact, current_version} detail envelope (contract delta #5).
  const response = await apiClient.get<CanvasVersion>(
