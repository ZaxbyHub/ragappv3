...
**Revision 2:** reconciled with an independent second-opinion review (GPT 5.6 Sol); see §6 for what changed and why.

---

## 1. The ask

A new sidebar section ("Draft Room") where the user uploads one or more documents and the system **rewrites them into high-quality prose, using the selected vault as a knowledge source**, with anti-AI-slop procedures modeled on the `opencode-newsroom` plugin — i.e., a real newsroom editorial pipeline: research → plan → write → copy edit → fact check → standards review, with hard quality gates between stages.
...
| **Stage A gates** (`pre_check_batch`) | **Deterministic** checks: 19-pattern banned-AI-vocab scan, sentence-length stats, passive-voice density, Flesch Reading Ease — each pass/warn/fail | Ports to ~200 lines of pure Python (`draft_quality.py`); lexicon scan as a hard gate, statistical heuristics as advisory signals (§5.8) |
...
**The two-layer anti-slop design is the core insight to preserve:** the *writer prompt* prevents slop at generation time (banned lexicon + structural rules), and the *review gates + deterministic lint* catch what slipped through — with bounded revision loops so the pipeline always terminates.
...
2. **New draft**: pick vault (reuse `VaultSelector`; active vault as default) → upload source document(s) (reuse `UploadDropzone`) → fill the **assignment brief**:
   - piece type (rewrite / article / report / brief), target audience, tone/voice, length target, content tier (1–3);
   - **a role for every uploaded source**: `manuscript` (preserve & improve) / `reference` (trusted facts) / `style exemplar` (imitate voice, never trust content) / `background` (summarizable) / `challenge` (claims to be verified against the vault, not trusted). Without this the system cannot know whether to preserve, verify, contradict, or imitate a document;
   - conflict policy (prefer vault / prefer source / surface both).
3. **Pipeline view** (the stage rail): Assignment → Research → Outline → Draft → Fact desk → Copy desk → Standards → Ready, driven by job SSE events + status polling, showing per-stage verdicts, retry counts, and warnings. **Every stage produces an inspectable artifact** — research packet, approved outline, per-section drafts, claim ledger, findings — never a silent jump from sources to polished prose.
...
drafts             id, vault_id → vaults, title, brief_json (incl. per-source roles),
                   tier, status ('assembling'|'queued'|'running'|'needs_review'|
                   'ready'|'failed'|'archived'), created_by → users, created_at, updated_at
draft_sources      draft_id → drafts, file_id → files, role
...
POST   /draft-room/drafts/{id}/sources         upload source file(s) → parse-only      [owner, csrf]
POST   /draft-room/drafts/{id}/compile         enqueue draft_jobs row → {job_id}       [owner + vault read, csrf, 202]
GET    /draft-room/drafts?vault_id=            list (own drafts)                       [vault read]
GET    /draft-room/drafts/{id}                 detail incl. latest revision + QA       [owner]
GET    /draft-room/drafts/{id}/status          job/stage progress (polling)            [owner]
GET    /draft-room/events                      fetch-SSE job events (wiki-events shape)[owner]
PUT    /draft-room/drafts/{id}                 edit brief / approve / archive          [owner, csrf]
POST   /draft-room/drafts/{id}/promote         index result/source into vault          [vault write, csrf]
...
Frontend: `lib/api/draftRoom.ts` on the shared `apiClient`; a `DraftRoomPage` modeled on `DocumentsPage` (reuse visual primitives and the three-pane layout *pattern* from `ChatShell`, but **dedicated stores** — draft state is not chat state; do not reuse `useChatStore`); nav updates in the four established places.
...
4 LINT        deterministic draft_quality.py — banned lexicon (hard gate), plus
              advisory signals: burstiness variance, ¶-start transition counts,
              passive %, Flesch, same-opener runs; lexicon fail → targeted rewrite (≤2)
5 COPY DESK   LLM (structured JSON verdict per dimension) → writer revision loop
              (≤ draft_qa_retry_limit). Any SEMANTIC change loops back through FACT
6 FACT DESK   atomic-claim ledger (§4.5): per-claim vault re-retrieval → Supported /
              Contradicted / Ambiguous / Stale / Unsupported / Opinion, each with
              exact passage + chunk reference; quote-fidelity gate (exact or marked
              paraphrase); Contradicted/Unsupported → targeted revision or loop back
              to RESEARCH for more evidence; residual issues → findings, not silence
7 STANDARDS   LLM (reframed humanizer, §5.7): stock phrasing, uniform rhythm, hedging,
              inflated significance, lost nuance/uncertainty, silent fact removal,
              style divergence from exemplars → targeted rewrites → re-run LINT
...
**"Anti-AI-slop" is a positive editorial standard, not detector evasion.** AI-writing detectors are unreliable in both directions (OpenAI retired its own classifier citing a 26% true-positive / 9% false-positive rate; NIST's GenAI evaluation reported generators defeating every tested detector), so "passes GPTZero" is the wrong optimization target. The right target is the checklist real desks use: unsupported facts/numbers/names/causal claims, altered or invented quotes, vague attribution ("experts say"), stock openings and canned conclusions, inflated significance, mechanically uniform rhythm and repeated sentence templates, lost nuance/uncertainty/limitations, silent removal of material facts, style divergence from approved exemplars. This mirrors AP/Reuters guidance: treat generative output as unvetted source material; accuracy over speed; quotes are sacrosanct.

- **`draft_quality.py`** (new, pure Python, CI-testable): port `pre_check_batch` + add burstiness variance, opener-repetition, transition counting, "AI triad" detection. Banned lexicon = union of the plugin's three lists, stored as a configurable setting. Lexicon = hard gate; statistical heuristics = advisory signals feeding the copy desk (they are lint, not verdicts).
- **The fact desk needs more than today's citation validator.** `citation_validator.py` verifies *label integrity* and scores confidence via *lexical overlap* — a valid `[S3]` label does not prove the cited passage entails the claim. The fact desk therefore decomposes output into atomic claims (FActScore-style) and re-retrieves per claim, recording the exact supporting passage + document identity + chunk reference for each Supported verdict. The existing validator remains the final label-integrity pass.
- **Role prompts** ported from the plugin (writer/copy-editor/fact-checker/managing-editor; standards desk rewritten per §5.7), adapted to cite vault evidence. Candidate home: the existing `prompt_versions` infrastructure rather than string constants.
...
**5.2 Draft sources are project-private and parse-only — never auto-indexed into the vault.** Otherwise the rewrite retrieves its own source as "grounding" and drafts-in-progress pollute chat/wiki for every vault user. Reuse the upload+parse machinery with an indexing-skip flag; a deliberate **promote** action (vault `write`) indexes a source or finished draft later. Both reviews converged here. Add a retention/deletion policy for project attachments.

**5.3 Rewrite vs compose are both real modes.** Rewrite preserves the manuscript's structure and claims (outline derived, critic gate trivial) — the cheaper, faster path. Compose plans its own structure from sources + vault. The brief's piece type selects; MVP leads with rewrite while keeping the research-packet and outline artifacts visible in both modes.
...
**5.8 Deterministic lint is a gate only where it's precise.** The banned-lexicon scan is cheap, configurable, and near-zero-false-positive on flagrant patterns ("delve," "tapestry," "it's important to note") — keep it as a hard gate honoring the user's explicit anti-slop ask. Passive-voice %, Flesch, and sentence statistics are noisy — advisory signals in the QA report and copy-desk input, not blockers.
...
Adopted from that review into this revision: per-source roles in the brief (§4.1); the standards-desk reframe replacing detector-evasion (§5.7); the atomic-claim ledger with entailment-oriented verdicts and quote-fidelity gate (§4.5, §4.4 stage 6); copy-desk semantic changes looping back through fact review; the permissions revision (§5.4); universal human Ready (§5.5); honest gate labeling; Phase 0 gold corpus (§8); export auditing, provider-content policy, and the editor-gap risk (§7).
...
Retained despite that review's skepticism: the banned-lexicon **hard** gate (§5.8 — Sol demotes all deterministic checks to lint; this doc keeps the one precise check as a gate) and optional per-role model routing in later phases (§4.6 — this user demonstrably operates heterogeneous model rosters; the *default* remains the app's configured models, where both reviews agree).
...
5. **Vault-empty grounding** — degrade to source-only rewrite with an explicit "no vault grounding" banner (CRAG evaluator reuse), never hallucinated citations.
...
8. **Privacy / provider policy** — project attachments flow to whatever LLM endpoint is configured; the settings surface should state this and (later) allow restricting Draft Room to specific endpoints.
...
- **Phase 0 — Define quality before building.** Assemble a small gold corpus: single-manuscript rewrite; conflicting references; stale vs current sources; exact quotations; OCR-degraded input; opinion mixed with fact; embedded malicious instructions; sensitive/cross-vault docs; 2–3 approved voices. Decide source-priority, retention, provider policy, and what **Ready** means. This corpus later drives the eval harness.
- **Phase 1 — Honest vertical slice.** Tables + migration; project CRUD + brief (incl. source roles); parse-only uploads; research packet + outline artifacts; rewrite with version history; original/rewrite comparison (simplest viable diff); LINT; manual edits; human approval; `.md` export; durable jobs + progress. Ships labeled **"Draft generated — not fact-checked."** *~1.5–2k backend LOC, ~1–1.5k frontend.*
- **Phase 2 — Editorial quality gates.** Atomic-claim ledger (`draft_claims`/`draft_claim_sources`/`draft_findings`); quote verification; contradiction + freshness findings; copy desk + standards desk with semantic-change loop-back; accept/reject findings; tiers; full audit trail (prompt/model/source/version per run); SSE event stream; `[D#]` labels.
- **Phase 3 — Product hardening.** Voice profiles/exemplars; `.docx`/PDF export; per-role model routing; cost/token budgets + stage caching; retention controls; promote-to-vault/wiki; slop-metric eval dashboard against the gold corpus; collaboration/named approvers.
...
Plugin claims: verified first-hand (all agent prompts, guardrails, pre-check tool, state machine, config read at commit `690fcea`; evidence/knowledge write-path wiring checked by grep in revision 2). ragappv3 claims: mapped by three parallel explorer passes, then re-verified by an independent reviewer pass that opened every cited file — all 22 load-bearing claims confirmed. Precision notes from that pass: `files.parsed_text` persistence is at `document_processor.py:2988–2991`; `wiki_compile_jobs` lacks the `input_json`/`retry_count` columns that `kms_compile_jobs` has (hence §4.2 clones the **kms** table shape). Revision-2 additions verified directly: no diff/editor library in the frontend; no callers of the plugin's `saveEvidence`/`addKnowledge`. External claims about AI-detector reliability (OpenAI classifier retirement figures, NIST GenAI evaluation) are reproduced from the second-opinion review and public reporting; they motivate framing only and no design element depends on their exact numbers. Anchors use `~` where line numbers may drift.

## 10. Open questions (blocking a SPEC)

Resolved by convergence of two independent investigations (still the user's call, but both reviews agree): parse-only project-private sources with explicit promote; rewrite-first MVP with visible research/outline artifacts; human-only Ready; standards-desk reframe.

Still genuinely open:
1. Permissions: confirm the revised read→private-draft / write→promote model (§5.4) — it reverses this doc's first-version recommendation.
2. Export targets: `.md` only for MVP, or is `.docx` required day one?
3. Tier semantics in Phase 1, or defer tiers to Phase 2 with the claim ledger?
4. Diff/editor library choice for the center pane (nothing exists today).
5. Retention policy for project attachments (delete on draft archive? configurable TTL?).
6. Should the deterministic lint eventually run on *chat* answers too? It's independently useful.
7. Phase 0 gold corpus: who curates it, and which 2–3 house voices should it encode?
