...
- Deployed image: `sha256:1714ba3ba5c657d626ae337a619125dbaca9ee0fee905c30affc298233dbe3e3` (built on R640AI from the qualification branch at the build commit above — master `54f8682e` + this PR's docs and the reindex probe fix; container healthy; the #647 `FALLBACK_SCORE_FLOOR` marker and the probe fix both verified present in the deployed code). Qualification ran in two stages: the full battery at `edd2c741…` (image 65aad900) and the blocker re-runs at the fixed build above, which contains the first as an ancestor. Stage 3 (2026-09-21, issue #652): the mixed-source-compose gate was re-qualified on the #652 fix build — branch `zcode/652-thinking-client-timeout-streaming` at `f438e0100181f975e4a5255b70cf5cefcb52ef16` (image tag `ragappv3-knowledgevault:fix652-f438e010`, id 3619ddac9319), which contains this build as an ancestor — see `requa5_mixed_compose.transcript.json`; the R640 was restored to the stage-2 image afterwards.
- Generator provenance: the data JSON and matrix tables were emitted by generator scripts preserved in the issue trace (`.agents/issue-traces/229-meridian-experience-qualification/scratch/build_data_json_r2.py` / `build_matrix.py`, intentionally uncommitted per the trace-exclusion design); integrity is enforced post-hoc by the frozen C12 evidence-citation check and the `test_meridian_roadmap_contract.py` guardrail, which re-validate the committed tree.
...
| draft-compose-rewrite | pass | docs/eval/2026-09-meridian-qualification-evidence/requa5_mixed_compose.transcript.json — mixed-source compose full lifecycle post-#652; source-only-rewrite leg via requa3c_drafts_vault2.transcript.json (draft 7, 33 citations) |
| findings-compare | pass | quality report→eval case→compare, fact_coverage delta 1.0 |
| export | pass | canvas download + manifest export; draft export 200 post-#652 (requa5_mixed_compose.transcript.json) |
...
2. **RESOLVED — Reindex failure (reindex dimension-probe bug).** The deterministic post-outage reproduction (docs/eval/2026-09-meridian-qualification-evidence/repro_reindex_pre_fix.log) proved the failure was NOT the #645 checkout class but the #529-introduced probe tuple-unpack bug; this PR fixes it (backend/app/services/background_tasks.py + backend/tests/test_issue229_reindex_dimension_probe.py, RED-on-revert proven) and reindex now completes end to end (docs/eval/2026-09-meridian-qualification-evidence/requa4_reindex_cleanvault.transcript.json job 463 completed; vault-2 job 396 pass over 13/15 files with 2 pre-existing corrupt corpus files failing per-file parse). #645 remains open for its own scope.
3. **RESOLVED — Draft gates on the restored server.** Source-only rewrite: PASS (full lifecycle, draft 7 job 28: revision, findings, export). Mixed-source compose: PASS after the #652 fix (settings-backed per-mode request timeouts + streaming Draft Room stage calls) — post-fix requalification (docs/eval/2026-09-meridian-qualification-evidence/requa5_mixed_compose.transcript.json): a fresh mixed-source compose draft completed all nine stages against the deployed thinking server (final attempt 17.0 min inside DRAFT_JOB_TIMEOUT_SECONDS=7200; the 70.8 min first attempt ran every stage call on the streaming transport with no 300 s cap — provable bounds: research stage 23.7 min across 2 calls, longest single call ~14.0 min, outline/draft calls under 300 s each — and its ~39 min tail was the copy stage's transient ladder against a failing third-party editorial backend), revision with 59 citations, fact_status passed, export 200. Failure-era history retained in requa3c_drafts_vault2.transcript.json + requa3_draft_latency_cause.md. Environment note: the third-party editorial backend (192.168.1.145) dropped its long copy streams during the requalification window; the operator-configurable editorial endpoint was switched to the local minicpm5-2b for the run (deviation recorded in the transcript) — an environment condition, not an application defect.
