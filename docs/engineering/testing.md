...
- **Per-file `unstructured` import stubs are load-bearing for CI**, not redundant boilerplate — CI installs the reduced `requirements-ci.txt` (see §4), which omits `unstructured` (and `sentence-transformers`). `lancedb` and `pyarrow` are now installed on CI for real (added for the issue-#513 acceptance checks under `tests/issue513_checks/`, which exercise the real LanceDB async surface), so their per-file stubs are local-machine fallbacks rather than CI requirements. Removing an `unstructured` stub can still break collection in CI even though the file passes locally with the full deps installed.
...
- **Quality contracts job:** all seven contract scripts, in CI order: `scripts/check_runtime_contract.py`, `scripts/check_config_contract.py`, `scripts/check_settings_consumers.py`, `scripts/check_pr_scope_drift.py`, `scripts/check_sast_baseline.py`, `scripts/check_secretscan.py`, `scripts/check_test_collection_scope.py` (mirrored by the `justfile` `quality-contracts` recipe).
