...
- Runtime re-pin: **Node.js 22.14.0 LTS (minor-exact) + Python 3.11** across
...
- **INSTALLATION.md rebuilt on the maintained flow** (validated by the new
  ...
  job): versions aligned to the runtime contract (Node 22.14 / Python 3.11
...
- **Local dev Node floor is now 22.14** — `frontend/package.json` engines
  enforces `>=22.14.0` (CI, both Dockerfiles and CONTRIBUTING are aligned;
